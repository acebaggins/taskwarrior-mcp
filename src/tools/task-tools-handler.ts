...
export class TaskToolsHandler {
  ...
  registerTools(server: McpServer): void {
    ...
    server.tool(
      ...
      "Create a new task in Taskwarrior. This is the primary way to add new tasks to your task list. You can specify the task's description, project, tags, priority, and various dates.",
      {
        ...
        tags: z
          .array(z.string())
          .optional()
          .describe('Tags to categorize and filter tasks. Useful for grouping similar tasks.'),
      ...
      async params => {
        try {
          const task = await this.taskService.createTask(params);
          return {
            content: [
              {
                type: 'text',
    ...
    server.tool(
      ...
      async params => {
        try {
          const task = await this.taskService.startTask(params.id);
          if (params.note) {
            await this.taskService.addAnnotation(params.id, params.note);
          }
          return {
            content: [
              {
    ...
    server.tool(
      ...
      async params => {
        try {
          const task = await this.taskService.stopTask(params.id);
          if (params.note) {
            await this.taskService.addAnnotation(params.id, params.note);
          }
          return {
    ...
    server.tool(
      ...
      async params => {
        try {
          const task = await this.taskService.completeTask(params.id);
          if (params.note) {
            await this.taskService.addAnnotation(params.id, params.note);
          }
          return {
    ...
    server.tool(
      ...
      'Add a note or annotation to a task. Annotations are timestamped and useful for tracking progress or changes over time. Regular notes are good for general information.',
      {
        ...
        isAnnotation: z
          .boolean()
          .optional()
          .describe('If true, adds the note as an annotation (timestamped). If false, adds as a regular note.'),
      },
      async params => {
        try {
          const task = await this.taskService.addAnnotation(params.id, params.note);
          return {
            content: [
              {
                type: 'text',
    ...
    server.tool(
      ...
      async params => {
        try {
          ...
          const task = await this.taskService.updateTask(id, update);
          return {
            content: [
              {
                type: 'text',
    ...
    server.tool(
      ...
      async params => {
        try {
          const tasks = await this.taskService.listTasks(params);
          return {
            content: [
              {
                type: 'text',
